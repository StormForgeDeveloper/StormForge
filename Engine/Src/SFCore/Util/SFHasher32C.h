...
namespace SF {
	...
	// Hasher class for crc32c
	class Hasher_Crc32C
	{
	public:


		//	The Rocksoft^tm Model CRC Algorithm Table Generation
		//	Program V1.0 using the following model parameters:  
		//                                                      
		//    Width   : 4 bytes.                                
		//    Poly    : 0x1EDC6F41L                             
		//    Reverse : TRUE.                                   
		static constexpr uint32_t stm_CrcTable[256] = {
	...
	private:
		...
		static constexpr uint32_t Crc32C(uint32_t crc, const uint8_t* pBuff, size_t length)
		{
			...
			while (length-- > 0)
			{
				crc = (crc >> 8) ^ Hasher_Crc32C::stm_CrcTable[(crc ^ (*pBuff++)) & 0xFF];
			}
			return crc;
		}
	...
	public:
		...
		static constexpr uint32_t Hash(const char* data)
		{
			return data == nullptr
				? 0
				: MakeNonZero(Crc32C(0xffffffff, data) ^ 0xffffffff);
		}
