...
namespace SF
{
 	namespace Policy
	{
		...
		class NetSvrPolicyGame
		{
			...
			// Cmd: Player connected from a login server and moved to game server
			Result JoinGameServerRes( const Result &InResult, const char* InNickName, const uint64_t &InGameUID, const uint64_t &InPartyUID, const AccountID &InPartyLeaderID, const MatchingQueueTicket &InMatchingTicket );
			// Cmd: player complition statues
			...
			// Cmd: Remove friden form the friend list
			Result RemoveFriendRes( const Result &InResult, const AccountID &InFriendID );
			// S2C: Friend removed
			...
			// Cmd: Game game play information
			Result GetGamePlayerInfoRes( const Result &InResult, const AccountID &InPlayerID, const int16_t &InLevel, const int32_t &InTotalPlayed, const int32_t &InWinPlaySC, const int32_t &InWinPlaySM, const int32_t &InWinPlaySS, const int32_t &InLosePlaySC, const int32_t &InLosePlaySM, const int32_t &InLosePlaySS, const int32_t &InWinPlayNC, const int32_t &InWinPlayNM, const int32_t &InWinPlayNS, const int32_t &InLosePlayNC, const int32_t &InLosePlayNM, const int32_t &InLosePlayNS, const int32_t &InWeeklyWin, const int32_t &InWeeklyLose );
			...
			// Cmd: Create Party
			Result CreatePartyRes( const Result &InResult, const uint64_t &InPartyUID );
			// Cmd: Join party
			Result JoinPartyRes( const Result &InResult, const uint64_t &InPartyUID, const AccountID &InPartyLeaderID, const Array<uint8_t>& InChatHistoryData );
			...
			// Cmd: Join to a game
			Result JoinGameRes( const Result &InResult, const uint64_t &InInsUID, const uint32_t &InTimeStamp, const uint8_t &InGameState, const uint8_t &InDay, const uint8_t &InMaxPlayer, const uint8_t &InPlayerIndex, const uint8_t &InPlayerCharacter, const uint8_t &InRole, const uint8_t &InDead, const Array<uint8_t>& InChatHistoryData, const Array<uint8_t>& InGameLogData );
			...
			// Cmd: Player. reveal a player
			Result GameRevealPlayerRes( const Result &InResult, const Array<AccountID>& InRevealedPlayerID, const Array<uint8_t>& InRevealedRole, const uint64_t &InTotalGem, const uint64_t &InTotalGameMoney );
			// Cmd: Player. revive himself
			...
			// Cmd: Create or Join Chat channel
			Result CreateOrJoinChatChannelRes( const Result &InResult, const uint64_t &InChatUID );
			// Cmd: Join
			Result JoinChatChannelRes( const Result &InResult, const uint64_t &InPartyUID, const AccountID &InPartyLeaderID );
			// S2C: Player Joined event
			...
			// Cmd: Leave ChatChannel command
			Result LeaveChatChannelRes( const Result &InResult );
			...
			// Cmd: Kick player from the ChatChannel
			Result ChatChannelKickPlayerRes( const Result &InResult );
			...
			// Cmd: Party chatting
			Result ChatChannelChatMessageRes( const Result &InResult );
			// S2C: ChatChannel Chatting message event
			Result ChatChannelChatMessageS2CEvt( const AccountID &InSenderID, const char* InSenderName, const char* InChatMessage );
			// Cmd: Give my stamina to other player
			Result GiveStaminaRes( const Result &InResult, const AccountID &InTargetPlayer, const uint64_t &InTimeStamp );
			// Cmd: For debug, Change configue preset
