...
// Author : KyungKun Ko
//
// Description : Online client interface
//	
//
////////////////////////////////////////////////////////////////////////////////
...
{
    if (nativeHandle == 0)
        return (uint)EAccountRole::Player;

    auto pOnlineClient = NativeToObject<OnlineClient>(nativeHandle);

    return static_cast<uint32_t>(pOnlineClient->GetAccountRole());
}
	...
	OnlineClient::ONLINE_TASK_FINISHED_CALLBACK onTaskFinished)
{
	...
	{
            onMessageFunc(pMsgData->GetMessageID().ID, pMsgData->TransactionId.ID, pMsgData->GetTransactionResult(), pMsgData->GetPayloadSize(), pMsgData->GetPayloadPtr());
	};
