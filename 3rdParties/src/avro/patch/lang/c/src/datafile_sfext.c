...
static int file_writer_create_writer(avro_writer_t file_writer, int should_close, avro_schema_t schema, avro_file_writer_t w, size_t block_size)
{
	int rval;
			...
			const char *codec, size_t block_size)
{
	...
	rval = file_writer_create_writer(file_writer, path, should_close, schema, w, block_size);
	if (rval) {
		avro_codec_reset(w->codec);
		avro_freet(struct avro_codec_t_, w->codec);
		avro_freet(struct avro_file_writer_t_, w);
...
int avro_file_reader_reader(avro_reader_t reader, avro_file_reader_t * reader)
{
	int rval;
