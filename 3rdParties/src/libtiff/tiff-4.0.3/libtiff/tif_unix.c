...
_tiffReadProc(thandle_t fd, void* buf, tmsize_t size)
{
	...
	return ((tmsize_t)fread(buf, 1, size_io, (FILE*)fd));
}
...
_tiffWriteProc(thandle_t fd, void* buf, tmsize_t size)
{
	...
	return ((tmsize_t)fwrite(buf, 1, size_io, (FILE*)fd));
}
...
_tiffSeekProc(thandle_t fd, uint64 off, int whence)
{
	...
	return ((uint64)fseek((FILE*)fd, off_io, whence));
}
...
/* for cygwin and mingw */
	...
	tif = TIFFFdOpen((thandle_t)fd, (mbname != NULL) ? mbname : "<unknown>",
			 mode);
