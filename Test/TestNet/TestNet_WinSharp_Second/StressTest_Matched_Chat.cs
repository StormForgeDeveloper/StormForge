...
namespace TestNet.WinSharp
{
    public class StressTest_Matched_Chat
    {
        ...
        float mHeartBitTime = 0;
        public bool IsPartyBoss { get; set; }

        public bool IsPutRecvChatLog { get; set; }
        ...
        int mTotalRecvedPartyChatMessageCount = 0;
        ...
        System.Action<StressTest_Matched_Chat, UInt64> mPartyChatOneKBRecivedTen = null;

        System.Action<StressTest_Matched_Chat, int, UInt64> mPartyChatOneKBRecivedSingle = null;
        ...
        {
         //   PrintStatus(0, "HandlePartyChatMessageRes");
            var Result = message.GetValue<Result>("Result");
            if (Result.IsFailed)
            {
        ...
        Dictionary<UInt64, List<PartyChatMessageRecvInfo>> mRecvChatHistorys =
            new Dictionary<UInt64, List<PartyChatMessageRecvInfo>>();
        ...
        public long GetChatRecved_AverageValue_Single(DateTime SendTime, UInt64 SenderID, int MessageIndex)
        {
            if (!mRecvChatHistorys.ContainsKey(SenderID))
                return 0;

            if (mRecvChatHistorys[SenderID].Count < MessageIndex)
                return 0;

            double TotalMilliseconds = (mRecvChatHistorys[SenderID][MessageIndex-1].RecvTime - SendTime).TotalMilliseconds;
            return (long) TotalMilliseconds;

            //for (int i = 0; i < mRecvChatHistorys[SenderID].Count; i++)
            //    TotalMilliseconds += (mRecvChatHistorys[SenderID][i].RecvTime - SendTime).TotalMilliseconds;

            //long AverageValue = (long)(TotalMilliseconds / mRecvChatHistorys[SenderID].Count);
            //return AverageValue;
        }

        public long GetChatRecved_AverageValue(DateTime SendTime, UInt64 SenderID)
        {
            if (!mRecvChatHistorys.ContainsKey(SenderID))
                return 0;
        ...
        void HandlePartyPlayerJoinedS2CEvt(SFMessage message)
        {
            ...
            PrintStatus(1, "HandlePartyPlayerJoinedS2CEvt Owner ={0}", m_LoginID);
        ...
        void HandlePartyPlayerLeftS2CEvt(SFMessage message)
        {
            ...
            PrintStatus(1, "HandlePartyPlayerLeftS2CEvt Owner ={0}", m_LoginID);
