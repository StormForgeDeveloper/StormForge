...
static int Process()
{
	const char* inputPath = SF::AppConfig::GetSetting("in");
	...
	if (testResult == SF::StringCrcDB::FileHeaderCheckResult::Successed)
	{
		...
		{
			SFLog(Engine, Error, "SFStringCrcBin: Failed to read input binary {0}", outputPath);
			return -1;
		}
	...
	{
		SFLog(Engine, Error, "SFStringCrcBin: Failed to read input binary, invalid file version {0}", outputPath);
		return -2;
	}
	else
	{
		...
		{
			SFLog(Engine, Error, "SFStringCrcBin: Failed to read data from input text {0}", outputPath);
			return -2;
		}
